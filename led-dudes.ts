...
import { AbstractLedController, getLedController, Config as ControllerConfig } from './led-controller'

type DudeMode = 'constant' | 'stripes' | 'test' | 'classic' | 'fm'
type BlendMode = 'SUM' | 'AVERAGE' | 'MULTIPLY' | 'DODGE' | 'BURN' | 'REPLACE' | 'DEFAULT'
...
type ScheduleTime = [number, number]
...
exports.start = (config: Config, opts: Opts) => {
...
class DudeManager {
    ...
    constructor(config: Config, { dummyMode, ignoreSchedule }: Opts) {
        ...
        this.dudes = config.dudes.map(dudeConfig => {
            ...
            lastDude = new Dude({ seed, ...dudeConfig, stripLength: config.stripLength })
            return lastDude
        })
        ...
        ;['SIGTERM', 'SIGINT'].forEach(event => process.on(event, () => {
            ...
            if (this.scheduleInterval !== undefined) clearInterval(this.scheduleInterval)
            this.controller.off()
            process.exit(1)
        }))
    }
    ...
    checkSchedule(): void {
        ...
        } else if (this.dudesInterval !== undefined && isAfter(now, this.currentScheduleRule.end)) {
        ...
        return setInterval(() => {
            ...
            for (let position = 0; position < this.config.stripLength; position++) {
                const color = <Rgb>this.dudes
                    .map((dude): [Dude, Rgb] => [dude, dude.positionTick(t, dt, position)])
                    .reduce((rgbSum: Rgb, [dude, rgb]) => blendColors(dude.blendMode, rgbSum, rgb), [0, 0, 0])
...
class Dude {
    ...
    constructor(config: DudeConfig) {
        ...
        if (this.mode !== 'constant' && this.mode !== 'stripes') {
            ...
            this.seed = _.defaultTo(config.seed, Math.random() * SEED_SCALE - 0.5 * SEED_SCALE)
            this.timeParam = 0
    ...
    timeTick(t: number, dt: number): void {
        ...
        if (this.mode === 'stripes') {
            ...
            if (this.timeParam === undefined || this.morphRate === undefined || this.seed === undefined) throw TypeError('this should never happen')
            ...
            if (this.seed === undefined || this.C === undefined || this.M === undefined) throw TypeError('this should never happen')
        ...
        if (this.strobe) {
            if (this.sinceFlip === undefined || this.strobePeriod === undefined) throw TypeError('this should never happen')
            this.sinceFlip += dt
    ...
    positionTick(t: number, dt: number, position: number): Rgb {
        ...
        if (this.mode === 'test') {
            if (!this.testColors || !this.testColors[position]) throw new TypeError('this should never happen')
            return this.testColors[position]
        }
        if (this.mode === 'constant') return this.rgb
        if (this.mode === 'stripes') {
            if (this.firstEdge === undefined) throw TypeError('this should never happen')
        ...
        let level = 1
        if (this.mode === 'classic') {
            if (this.positionParam === undefined || this.positionDelta === undefined || this.timeParam === undefined || this.scale === undefined || this.offset === undefined) throw TypeError('this should never happen')
            ...
            if (this.timeParam === undefined || this.seed === undefined || this.C === undefined || this.D === undefined || this.M === undefined) throw TypeError('this should never happen')
            level = Math.pow(scaleSine(fm(this.timeParam + FM_POSITION_SCALE / this.width * position + this.seed + this.slide, this.C, this.D, this.M)), this.power)
        }
...
const blendColors = (blendMode: BlendMode, base: Rgb, blend: Rgb): Rgb => {
    if (blendMode === 'DEFAULT') {
        ...
        else return <Rgb>base.map((value, index) => value + difference * blend[index])
    } else {
...
const scaleSine = (x: number, min = 0, max = 1): number => (Math.sin(x) * 0.5 + 0.5) * (max - min) + min
 ...
 * tVar2 - const2*t
 */
const fm = (x: number, C: number, D: number, M: number): number => Math.sin(C * x + D * Math.sin(M * x))
